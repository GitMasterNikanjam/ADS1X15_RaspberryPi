...
// Initialize the static instance map
std::map<int, ADS1115*> ADS1115::instanceMap;
...
bool ADS1X15::beginI2c()     // if ADS recognized -> return 1, else -> 0
{
  _ADS_handler = open(_i2cPath, O_RDWR);

  if (_ADS_handler < 0) 
  {
    errorMessage = "Failed to open I2C port. Are you sure about i2c file path?";
  ...
  {
    errorMessage = "Failed to set I2C slave address";
  ...
  {
    errorMessage = "ADS1x15 I2C address is not correct. Check device address!";
    close(_ADS_handler);
    return false;
  }
  ...
  if (!isConnected()) 
  {
    errorMessage = "ADS1X15 not recognized!";
    close(_ADS_handler);
    return false;
  }
...
bool ADS1X15::isConnected()     // if ADS recognized -> return 1, else -> 0
{
  ...
  if(i != 1)
  {
    errorMessage = "ADS1X15 not connected!";
    return false;
  }
...
{
  int16_t raw = _readRegister(ADS1X15_REG_CONVERT);
...
{
  uint16_t val = _readRegister(ADS1X15_REG_CONFIG);
...
{
  _writeRegister(ADS1X15_REG_LOW_THRESHOLD, lo);
}


int16_t ADS1X15::getComparatorThresholdLow()
{
  return _readRegister(ADS1X15_REG_LOW_THRESHOLD);
};


void ADS1X15::setComparatorThresholdHigh(int16_t hi)
{
  _writeRegister(ADS1X15_REG_HIGH_THRESHOLD, hi);
};


int16_t ADS1X15::getComparatorThresholdHigh()
{
  return _readRegister(ADS1X15_REG_HIGH_THRESHOLD);
};
...
void ADS1X15::_requestADC(uint16_t readmode)
{
  ...
  _writeRegister(ADS1X15_REG_CONFIG, config);
...
//$$$ this section changed from orginal source code
bool ADS1X15::_writeRegister(uint8_t reg, uint16_t value)
{
...
//$$$ this section changed from orginal source code
uint16_t ADS1X15::_readRegister(uint8_t reg)
{
  uint8_t bufRead[2];
...
bool ADS1115::setRdyPin(uint8_t rdyPin)
{

 if(rdyPin > 27)
  {
    errorMessage = "Error ADS1115: ALART/RDY pin can not set.";
    close(_ADS_handler);
    return false;
  }
  ...
  // Store the instance pointer in the map
  instanceMap[_rdyPin] = this;    
...
bool ADS1115::begin(void)
{
  ...
  if(_rdyPinFlag)
  {
    ...
    if (gpioInitialise() < 0) 
    {
      errorMessage = "Error ADS1115: pigpio library is not initialized. Please initialize it before creating GPIOInterruptHandler objects.";
      return false;
    }

    if(gpioSetMode(_rdyPin, PI_INPUT) != 0)
    {
      errorMessage = "Error ADS1115: Can not set ALART/RDY pin mode.";
      close(_ADS_handler);
      return false;
    }

    if(gpioSetPullUpDown(_rdyPin, PI_PUD_DOWN) != 0)
    {
      errorMessage = "Error ADS1115: Can not set ALART/RDY pin PullUpDown mode.";
      close(_ADS_handler);
      return false;
    }
    
    // Set up interrupt handler for GPS PPS pin.
    if(gpioSetISRFunc(_rdyPin, RISING_EDGE,0, _isrTrampoline) != 0)
    {
      errorMessage = "Error ADS1115: Can not set ISRFunc.";
      close(_ADS_handler);
      return false;
    }  

    //  SET ALERT RDY PIN trigger signal
    setComparatorThresholdHigh(0x8000);
    setComparatorThresholdLow(0x0000);
    setComparatorQueConvert(0);

    readADC(0);      //  trig first read from channel 0.
  }
...
void ADS1115::clean(void)
{
  close(_ADS_handler);

  if(_rdyPinFlag == true)
  {
    gpioSetMode(_rdyPin, PI_INPUT);
    gpioSetPullUpDown(_rdyPin, PI_PUD_OFF);
    gpioSetISRFunc(_rdyPin, RISING_EDGE,0, NULL);  // Set up interrupt handler for GPS PPS pin.
    // Remove the instance pointer from the map
    instanceMap.erase(_rdyPin);
    _rdyPinFlag = false;
  }
}
...
void ADS1115::_isrTrampoline(int gpio, int level, uint32_t tick) 
{
    // Find the instance associated with the GPIO pin and call the member function
    if (instanceMap.find(gpio) != instanceMap.end()) 
    {
        instanceMap[gpio]->_InterruptHandler(gpio, level, tick);
    }
}

void ADS1115::_InterruptHandler(int /*gpio*/, int /*level*/, uint32_t /*tick*/)
{
  _rdyFlag = true;
}

bool ADS1115::config(uint8_t gain, uint8_t dataRate , int rdyPin)
{
  /*
      Set ADS1115 reference voltage:
              GAIN
      0  =  +- 6.144V  default
      1  =  +- 4.096V
      2  =  +- 2.048V
  ...
  */
  ...
  /*
      Set ADS Conversion Rate.
      0  =  slowest -> 8 samples in one second
      7  =  fastest -> 860 samples in one second
      4  =  default -> 128 sample in one second
      Invalid values are mapped on 4 (default)
      Data Rate available: 8, 16, 32, 64, 128, 250, 475, 860
  */
